...
OptimInstanceQDO = R6Class("OptimInstanceQDO",
  public = list(
    ...
    initialize = function(objective, feature, search_space = NULL, terminator) {
      ...
      self$feature = assert_r6(feature, "Feature")
    ...
    print = function(...) {
      ...
      catf(str_indent("* Feature Function:", format(self$feature)))
      catf("* Search Space:")
  ...
  active = list(
    ...
    #' @field result_g (`numeric()`)\cr
    #' g part of the result.
    result_g = function() {
      unlist(private$.result[, self$feature$feature_function$codomain$ids(), with = FALSE])
    }
